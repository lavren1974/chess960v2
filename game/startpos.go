...
func chooseStartFEN(cfg *Config, r *rand.Rand) (string, error) {
    if cfg.StartFEN != "" { return cfg.StartFEN, nil }
...
func chess960FEN(index int) string {
    if index < 0 { index = 0 }
    if index > 959 { index = index % 960 }
    squares := []int{0,1,2,3,4,5,6,7}
    back := make([]rune, 8)
    for i := range back { back[i] = '.' }
    ...
    if len(squares) != 3 { panic("internal squares len !=3") }
...
func removeFile(a []int, file int) []int {
    out := a[:0]
    for _, v := range a { if v != file { out = append(out, v) } }
    return out
}
func removeAt(a []int, i int) []int { return append(append([]int{}, a[:i]...), a[i+1:]...) }
func idxOf(a []int, v int) int { for i, x := range a { if x == v { return i } }; return -1 }

func combinations(a []int, r int) [][]int {
    if r != 2 { panic("only r=2 supported here") }
    var out [][]int
    for i := 0; i < len(a); i++ {
        for j := i+1; j < len(a); j++ { out = append(out, []int{a[i], a[j]}) }
    }
    return out
}
